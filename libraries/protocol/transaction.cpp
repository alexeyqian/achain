...
#include <achain/protocol/exceptions.hpp>
...
digst_type signed_transaction::merkle_digest() const{
...
digest_type transaction::sig_digest(const chain_id_type& chain_id) const{
    ...
    fc::raw::pack(end, *this);
    return enc.result();
}

void transaction::validate() const{
    FC_ASSERT(operation.size() > 0, "A transaction must have at least one operation", ("trx",*this));
...
const signature_type& signed_transaction::sign(
...
signature_type signed_transaction::sign(
    const private_key_type& key, 
...
void transaction::get_required_authorities( 
...
set<public_key_tpe> signed_transaction::get_required_signatures(
    const chain_id_type& chain_id,
