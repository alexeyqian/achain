...
#include <achain/protocol/operation_util_impl.hpp>

namespace achain { namespace protocol {
    struct is_merket_op_visitor{
    ...
    bool is_merkey_operation(const operation& op){
        return op.visit(is_market_operation());
    }
...
DEFINE_OPERATION_TYPE( achain::protocol::operation )