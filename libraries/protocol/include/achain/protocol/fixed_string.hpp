...
namespace fc { namespace raw {
    ...
    inline void pack(Stream& s, const achain::protocol::fixed_string<Storage>& u){
        pack(s, std::string(u));
    }
...
namespace fc{
    ...
    void to_variant(const achain::protocol::fixed_string<Storage>&s, variant& v){
        v = std::string(s);
    }

    template<typename Storage>
    void from_variant(const variant& v, achain::protocol::fixed_string<Storage>& s){
        s = v.as_string();
    }
}
